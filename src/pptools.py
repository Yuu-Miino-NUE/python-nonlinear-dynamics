...
def pp (ode_fun, tend, y0, params, tstart=0, tick=1e-2, **kwargs):
    ...
    plt.connect('key_press_event', lambda event: on_key_pressed(event, plt, cfg, params))
    ...
    while cfg.isRunning:
        sol = solve_ivp(ode_fun, (tstart, tend), y0, t_eval=tspan, rtol=1e-5, args=[params])

        if not cfg.only_map:
            plt.plot(sol.y[cfg.xkey, :], sol.y[cfg.ykey, :],
                linewidth=cfg.linewidth, color=cfg.traj_color,
                ls = "-", alpha=cfg.alpha)

        plt.plot(sol.y[cfg.xkey, -1], sol.y[cfg.ykey, -1], 'o',
            markersize = cfg.pointsize, color=cfg.point_color, alpha = cfg.alpha)
...
def on_key_pressed(event, plt, config, params):
    match event.key:
        ...
        case 'up' | 'down': # For parameter control
            ...
            params[config.param_keys[config.param_idx]] = round(params[config.param_keys[config.param_idx]] + step, 10)
            print(params)
...
def on_click(event, plt, config, x0):
	...
	plt.plot(x0[config.xkey], x0[config.ykey], 'o', markersize = 3,
        color="blue", alpha = config.alpha)
